...
import { loadProgress, addCollectedWord, completeMission } from '../hooks/useProgress'
import { TRANSPORT_WORLD } from '../data/words/transport'
...
export default function OnTheMove({ onBack, onBackpackOpen, showReward, profile, showZainetto }) {
  const audio = useAudio()
  const progress = loadProgress()
  const [currentMission, setCurrentMission] = useState(TRANSPORT_WORLD.missions[0])
  ...
  useEffect(() => {
    const msg = profile ? `Ciao ${profile.childName}! Benvenuto nel mondo dei trasporti!` : 'Benvenuto nel mondo dei trasporti!'
  ...
  const handleMissionComplete = () => {
    ...
    if (nextIndex < TRANSPORT_WORLD.missions.length) { setMissionIndex(nextIndex); setCurrentMission(TRANSPORT_WORLD.missions[nextIndex]); setCollected({}); setTapCompleted([]); setSelectedMatch(null) }
    else { const completeMsg = profile ? `${profile.childName} ha completato TUTTI i mondi! 🏆🎉` : 'Tutti i mondi completati! 🏆🎉'; setTimeout(() => { showLocalReward(completeMsg); audio.speakItalian(completeMsg); setTimeout(() => onBack(), 3000) }, 1000) }
  }

  const handleTap = (word) => { audio.speakWord(word.word); if (currentMission.type === 'tap' && !tapCompleted.includes(word.id)) { setTapCompleted([...tapCompleted, word.id]); addCollectedWord(word.id); if (tapCompleted.length + 1 >= TRANSPORT_WORLD.words.length) setTimeout(() => handleMissionComplete(), 1000) } }
  ...
  const renderExercise = () => {
    switch (currentMission.type) {
      case 'tap': return (<div className="absolute inset-0 flex flex-col items-center justify-center pt-20"><p className="text-white text-xl font-bold mb-8 drop-shadow-lg">{currentMission.instruction} ({tapCompleted.length}/{TRANSPORT_WORLD.words.length})</p><div className="flex flex-wrap gap-6 justify-center px-4">{TRANSPORT_WORLD.words.map(word => (<WordObject key={word.id} obj={word} onTap={handleTap} isCollected={tapCompleted.includes(word.id)} isTarget={!tapCompleted.includes(word.id)} missionActive={true} />))}</div></div>)
      case 'drag': return (<div className="absolute inset-0 flex flex-col items-center justify-center pt-20"><div className="flex flex-wrap gap-6 justify-center px-4">{TRANSPORT_WORLD.words.map(word => (<WordObject key={word.id} obj={word} onTap={() => audio.speakWord(word.word)} onDragStart={handleDragStart} isCollected={!!collected[word.id]} isTarget={currentMission.targetWords.includes(word.id)} missionActive={true} />))}</div>{draggingObj && (<div className="fixed pointer-events-none z-50 text-6xl" style={{ left: dragPos.x - 40, top: dragPos.y - 40, transform: 'scale(1.2)', filter: 'drop-shadow(0 8px 16px rgba(0,0,0,0.3))' }}>{draggingObj.emoji}</div>)}{sparkles.map(s => <Sparkle key={s.id} x={s.x} y={s.y} />)}</div>)
      case 'match': const targetWord = TRANSPORT_WORLD.words.find(w => currentMission.targetWords.includes(w.id)); const options = [targetWord, ...TRANSPORT_WORLD.words.filter(w => !currentMission.targetWords.includes(w.id)).slice(0, 2)]; const shuffled = [...options].sort(() => Math.random() - 0.5); return (<div className="absolute inset-0 flex flex-col items-center justify-center pt-20"><p className="text-white text-xl font-bold mb-8 drop-shadow-lg">{currentMission.instruction}</p><div className="flex gap-6 justify-center px-4">{shuffled.map(word => (<div key={word.id} className={`cursor-pointer transition-all duration-300 ${selectedMatch === word.id ? 'scale-110' : 'hover:scale-105'}`} onClick={() => handleMatchSelect(word)}><div className="text-6xl">{word.emoji}</div>{selectedMatch === word.id && currentMission.targetWords.includes(word.id) && (<div className="text-green-400 text-2xl mt-2">✓</div>)}</div>))}</div></div>)
  ...
  return (
    <div className={`relative w-full h-screen overflow-hidden bg-gradient-to-br ${TRANSPORT_WORLD.background}`}>
      ...
      {renderExercise()}
      <Backpack isOpen={false} animation="" collectedItems={progress.collectedWords.reduce((acc, w) => ({ ...acc, [w]: true }), {})} onToggle={onBackpackOpen} objects={TRANSPORT_WORLD.words} profile={profile} />
