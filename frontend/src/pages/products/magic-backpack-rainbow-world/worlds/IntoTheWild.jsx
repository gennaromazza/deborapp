...
export default function IntoTheWild({ onBack, onBackpackOpen, showReward, profile, showZainetto }) {
  ...
  const {
    ...
    nextMission,
    isComplete,
  } = useMissionSelector(WILD_WORLD, childAge)
  ...
  const handleMissionComplete = () => {
    ...
    if (isComplete) { unlockWorld('fruits'); const completeMsg = getDialog('world_complete', profile, { world: 'giungla' }); setTimeout(() => { showLocalReward(completeMsg); audio.speakItalian(completeMsg); setTimeout(() => onBack(), 3000) }, 1000) }
    else { nextMission() }
  }
  ...
  const handleTap = (word) => {
    ...
    if (currentMission.type === 'tap' && !tapCompleted.includes(word.id)) {
      ...
      if (tapCompleted.length + 1 >= WILD_WORLD.words.length) setTimeout(() => handleMissionComplete(), 1000)
    }
  }
      ...
      case 'drag': return (
        <div className="absolute inset-0 flex flex-col items-center justify-center pt-20 pb-32">
          ...
          <div className="flex flex-wrap gap-6 justify-center px-4">
            {WILD_WORLD.words.map(word => (<WordObject key={word.id} obj={word} onTap={handleTap} onDragStart={(e) => handleDragStart(e, word)} isCollected={!!collected[word.id]} isTarget={currentMission.targetWords.includes(word.id)} missionActive={true} audio={audio} />))}
          </div>
