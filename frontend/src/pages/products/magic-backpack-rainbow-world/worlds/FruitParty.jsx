...
export default function FruitParty({ onBack, onBackpackOpen, showReward, profile, showZainetto }) {
  ...
  const [currentMission, setCurrentMission] = useState(FRUITS_WORLD.missions[0])
  ...
  const handleMissionComplete = () => {
    ...
    if (nextIndex < FRUITS_WORLD.missions.length) { setMissionIndex(nextIndex); setCurrentMission(FRUITS_WORLD.missions[nextIndex]); setCollected({}); setTapCompleted([]); setSelectedMatch(null) }
    else { unlockWorld('transport'); const completeMsg = profile ? `${profile.childName} ha completato la festa della frutta! 🏆` : 'Mondo completato! 🏆'; setTimeout(() => { showLocalReward(completeMsg); audio.speakItalian(completeMsg); setTimeout(() => onBack(), 3000) }, 1000) }
  }

  const handleTap = (word) => { audio.speakWord(word.word); if (currentMission.type === 'tap' && !tapCompleted.includes(word.id)) { setTapCompleted([...tapCompleted, word.id]); addCollectedWord(word.id); if (tapCompleted.length + 1 >= FRUITS_WORLD.words.length) setTimeout(() => handleMissionComplete(), 1000) } }
  ...
  return (
    <div className={`relative w-full h-screen overflow-hidden bg-gradient-to-br ${FRUITS_WORLD.background}`}>
      ...
      {renderExercise()}
      <Backpack isOpen={false} animation="" collectedItems={progress.collectedWords.reduce((acc, w) => ({ ...acc, [w]: true }), {})} onToggle={onBackpackOpen} objects={FRUITS_WORLD.words} profile={profile} />
