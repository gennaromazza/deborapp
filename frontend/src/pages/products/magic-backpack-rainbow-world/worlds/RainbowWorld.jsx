...
export default function RainbowWorld({ onBack, onBackpackOpen, showReward, profile }) {
  ...
  const [currentMission, setCurrentMission] = useState(RAINBOW_WORLD.missions[0])
  ...
  useEffect(() => {
    const welcomeMsg = profile
      ? `Ciao ${profile.childName}! Benvenuto nel mondo dei colori!`
  ...
  const handleMissionComplete = () => {
    ...
    const successMsg = profile
      ? `Bravo ${profile.childName}! 🎉`
      : 'Bravo! 🎉'
    showLocalReward(successMsg)
    audio.speakItalian(successMsg)

    const nextIndex = missionIndex + 1
    if (nextIndex < RAINBOW_WORLD.missions.length) {
      setMissionIndex(nextIndex)
      setCurrentMission(RAINBOW_WORLD.missions[nextIndex])
      setCollected({})
      setTapCompleted([])
      setSelectedMatch(null)
    } else {
      unlockWorld('pets')
      const completeMsg = profile
        ? `${profile.childName} ha completato il mondo dei colori! 🏆`
        : 'Mondo completato! 🏆'
      setTimeout(() => {
  ...
  const handleDragStart = (e, obj) => {
    ...
    const handleEnd = () => {
      ...
      const backpackY = window.innerHeight - 120
      const backpackX = window.innerWidth / 2
      const dist = Math.sqrt(Math.pow(dragPos.x - backpackX, 2) + Math.pow(dragPos.y - backpackY, 2))

      if (dist < 150 && currentMission.targetWords.includes(obj.id)) {
        setCollected(prev => ({ ...prev, [obj.id]: true }))
        addSparkle(dragPos.x, dragPos.y)
      ...
      case 'drag':
        return (
          <div className="absolute inset-0 flex flex-col items-center justify-center pt-20">
            <div className="flex flex-wrap gap-6 justify-center px-4">
              {RAINBOW_WORLD.words.map(word => (
                <WordObject
                  key={word.id}
                  obj={word}
                  onTap={() => audio.speakWord(word.word)}
            ...
            {draggingObj && (
              <div className="fixed pointer-events-none z-50 text-6xl" style={{ left: dragPos.x - 40, top: dragPos.y - 40, transform: 'scale(1.2)', filter: 'drop-shadow(0 8px 16px rgba(0,0,0,0.3))' }}>
                {draggingObj.emoji}
              </div>
            )}
